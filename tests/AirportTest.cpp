...
TEST(Airport, addTransport){
   ...
   t1.distance = 15; t2.distance = 15 ; t3.distance = 13;
   t1.time = v1; t2.time =v1; t3.time = v1;
   set<Transport> s1;
   s1.insert(t1); s1.insert(t2); s1.insert(t3);

   Airport a2("Humberto Delgado",s1);
   set<Transport>::iterator it = a2.getTransports().begin();
   EXPECT_EQ(it->distance,13);
